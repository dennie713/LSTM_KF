...
def RawData(t):
    ...
    # acc
    x_a = 0.1
    x_a1 = (- 0.5 * 1**2 * np.sin(1*t)) * x_a + 0
    C = 0.5
    y_a = C * x_a1
    x_a = x_a1

    x_p1_addNoise = x_p1 + np.random.normal(0, 0.001, np.array(x_p1).shape)
...
if __name__ == "__main__":
    ...
    np.savetxt('sim_2/x_data_all.txt', x_data_all, delimiter=' ')
    np.savetxt('sim_2/P_data_all.txt', P_data_all, delimiter=' ')
    np.savetxt('sim_2/raw_data_all.txt', raw_data_all, delimiter=' ')
