...
import KF, LSTM, load_test, getMonteCarlo
...
for epoch in range(epoch + 1):
    ...
    for i in range(0, traning_size, batch_size):
        ...
        x_y_pred_all.append(x_lstm_output.detach().cpu().flatten())
...
# x result儲存模型
torch.save(x_lstm_model.state_dict(), 'x_lstm_kf_model.pth')
print("-------- x Model saved successfully --------")
# P result儲存模型
torch.save(P_lstm_model.state_dict(), 'P_lstm_kf_model.pth')
